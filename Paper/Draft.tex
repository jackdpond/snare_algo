...
Several previous studies have examined what factors into MLB postseason performance. 
FanGraphs applied a clustering model to determine which bullpen matchups produced the highest probability of postseason success \cite{fangraphs_ps}. 
Furthermore, David Berri and Martin Schmidt applied regression models to show that regular season winning percentage is a relatively weak predictor of postseason outcomes \cite{berri_schmidt_2010}. 

%% Second Section 
\section{Data Set}

\par We use MLB game data from 2014-2025. The data includes variables such as pitcher skill level, team OOA, team WOBA, bullpen FIP, bullpen freshness, and whether the team is a wild card. The data set is strong in that the data is specifically geared towards games and understanding the impact of the wild card on the postseason.

\par Our final dataset incorporates data from 1) The pitcher logs for each game for each pitcher during the regular seasons and post seasons from 2014-2025, and 2) team level statistics (such as outs above average, weighted on-base average, whether the team is a wild card) for each team for each year from 2014 - 2025.
...
\par Our dataset required extensive data cleaning and feature engineering. As a precursor to the discussion, we will first define important terms and acronyms.

  \begin{enumerate}
  \item \textbf{FIP} Fielding Independent Pitching is a metric designed to capture how well a pitcher prevents runs independently of the rest of the defense.
  \item \textbf{Bullpen} Because pitching strains a pitcher's arm quickly, one pitcher will start and throw about 5-7 innings before being switched out for a \textit{relief pitcher}. The bank of pitchers ready to relieve is called the Bullpen.
...
\par We trained both Random Forest and XGBoost models to predict the outcome of a game based on the features of the teams playing. The primary goal of our modeling was not prediction accuracy, but rather to identify which features most strongly influence game outcomes in the postseason. These models work well for our problem because they are able to return feature importance scores that are interpretable and easy to understand.

\par With different iterations, our best Random Forest model achieved an F1 score of 0.6963, while our best XGBoost model achieved an F1 score of 0.701. The two models had similar feature importance ranking, with the top 15 features matching for both models.
